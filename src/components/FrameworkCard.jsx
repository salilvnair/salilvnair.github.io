...
import { ExternalLink, Github, CheckCircle2, ExternalLink as Launch } from 'lucide-react';

export default function FrameworkCard({ item, index }) {
  const { title, subtitle, description, github, tech, highlights, color, colorDim, badge, url } = item;
  ...
  return (
    <motion.div
      ...
      className="group relative flex flex-col rounded-2xl bg-th-card border border-th-border overflow-hidden shadow-card transition-all duration-300 cursor-pointer"
      style={{
        '--spot-x': '50%',
        '--spot-y': '50%',
        '--accent': color,
        ...
        <div className="pt-3 border-t border-th-border mt-auto space-y-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <a
              ...
              rel="noopener noreferrer"
              onClick={(e) => e.stopPropagation()}
              className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium transition-all group/link dark:text-slate-300 text-slate-600 dark:hover:text-white hover:text-slate-900 dark:bg-white/5 bg-slate-100 dark:hover:bg-white/10 hover:bg-slate-200/60 dark:border-white/10 border-slate-200 border rounded-xl"
            >
              <Github size={12} />
              GitHub
